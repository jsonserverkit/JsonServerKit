...
namespace JsonServerKit.Test
{
    ...
    public class Rsa
    {
        ...
        public void Main()
        {
            // https://stackoverflow.com/questions/41594683/encrypt-decrypt-in-c-sharp-using-certificate/41595289#41595289
            ...
            var privateKey = serverCertificate.GetRSAPrivateKey();
            var publicKey = serverCertificate.GetRSAPublicKey();
            ...
            try
            {
                ...
                string ciphertextBase64 = Base64Encoding(rsaEncryptionOaepSha1_x(serverCertificate, dataToEncrypt));
                ...
                byte[] decryptedtextByte = rsaDecryptionOaepSha1_x(serverCertificate, ciphertextReceived);
                Console.WriteLine("decryptedData: " +
                                  Encoding.UTF8.GetString(decryptedtextByte, 0, decryptedtextByte.Length));
            }
        ...
        public static byte[] rsaEncryptionOaepSha1_x(X509Certificate2 cert, byte[] plaintext)
        {
            var RSAalg = cert.GetRSAPublicKey();
            return RSAalg.Encrypt(plaintext, RSAEncryptionPadding.OaepSHA1);
        }
        ...
        public static byte[] rsaDecryptionOaepSha1_x(X509Certificate2 cert, byte[] ciphertext)
        {
            var RSAalg = cert.GetRSAPrivateKey();
            var usages = ((RSACng)RSAalg).Key.KeyUsage;
            return RSAalg.Decrypt(ciphertext, RSAEncryptionPadding.OaepSHA1);
        }
        ...
        {
            return
                "<RSAKeyValue><Modulus></Modulus><Exponent>AQAB</Exponent></RSAKeyValue>";
        }
